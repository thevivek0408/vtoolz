...
// Set worker path for PDF.js rendering on main thread
...
document.getElementById('split-btn').addEventListener('click', async () => {
    ...
    try {
        window.Utils.showToast('Creating new PDF...', 'info');

        const buffer = await window.PdfTools.readFileAsBuffer(currentFile);

        // Convert 1-based Set to 0-based sorted array
        const pages = Array.from(selectedPages).map(p => p - 1).sort((a, b) => a - b);

        const blob = await window.PdfTools.runPdfTask('SPLIT_PDF', {
            file: buffer,
            ranges: pages
        });

        window.Utils.downloadBlob(blob, 'split-document.pdf');
